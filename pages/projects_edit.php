...
<?php
 if(isset($_POST['projects-edit'])){
   $req_fields = array('projects-name','projects-lead','projects-category','projects-client','projects-description','projects-progress' );
   ...
   if(empty($errors)){
     ...
     $query .=" name='{$p_name}',lead_emp='{$p_lead}', category='{$p_cat}', client='{$p_client}', description='{$p_desc}', progress='{$p_progress}', project_url='{$p_url}' ";
     $query  .=" WHERE id ='{$project['id']}' ";
       $result = $db->query($query);
               if($result && $db->affected_rows() === 1){
                 ...
                 redirect('projects_edit.php?id='.$project['id'], false);
               }
          ...
          <div class="col-md-8">
            <div class="card">
              <div class="card-header">
                  <h4 class="card-title d-inline">Edit Project : <span class="text-info"><?php echo remove_junk($project['name'])?></span>
                 <div class="dropdown d-inline pull-right">
                  <button type="button" class="btn btn-link dropdown-toggle btn-icon" data-toggle="dropdown">
                    <i class="fa fa-cog"></i>
                  </button>
                  <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink">
                    <a class="dropdown-item" href="projects.php">Cancel</a>
              ...
              <div class="card-body">
                <form autocomplete="off" method="post" action="projects_edit.php?id=<?php echo (int)$project['id'] ?>">
                  <div class="row">
                      ...
                      <div class="form-group">
                        <label for="exampleInputEmail1">Client</label>
                            <select class="form-control" name="projects-client">
                              <option value="<?php echo remove_junk($project['client'])?>">Current Client : <?php echo remove_junk($project['client'])?></option>
